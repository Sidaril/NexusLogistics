...
public class MyCheckButton : MonoBehaviour
{
    ...
    public static MyCheckButton CreateCheckButton(float x, float y, RectTransform parent, int fontSize = 15)
    {
        ...
        if (child != null)
        {
            ...
            if (cb.labelText)
            {
                cb.labelText.material = null;
                var sh = cb.labelText.GetComponent<Shadow>();
                if (sh != null) DestroyImmediate(sh);
                var ot = cb.labelText.GetComponent<Outline>();
                if (ot != null) DestroyImmediate(ot);

                var newShadow = cb.labelText.gameObject.AddComponent<Shadow>();
